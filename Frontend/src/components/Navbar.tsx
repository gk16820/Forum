      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl tracking-tight text-slate-900">Forum</span>
          </Link>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 p-4 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <button 
                      ...
                      className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 rounded-xl transition-colors mt-2 shadow-lg shadow-green-600/20"
                    >
                      Search
                    </button>
                  </div>
              ...
              <>
                <div className="relative">
                  <button 
                    ...
                    className="text-slate-500 hover:text-green-600 transition-colors relative p-1"
                  >
                  ...
                  {showNotifications && (
                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        ...
                        {unreadCount > 0 && <span className="text-[10px] bg-green-100 text-green-600 px-2 py-0.5 rounded-full font-bold">{unreadCount} New</span>}
                      </div>
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.slice(0, 6).map(n => (
                            <div 
                              ...
                              className={`p-4 flex gap-3 hover:bg-slate-50 cursor-pointer transition-colors border-b border-slate-50 last:border-0 ${!n.isRead ? 'bg-green-600/5' : ''}`}
                            >
                              ...
                              {!n.isRead && <div className="w-2 h-2 bg-green-600 rounded-full self-center flex-shrink-0" />}
                            </div>
                          ))
                        )}
                      </div>
                      {notifications.length > 0 && (
                        <button 
                          ...
                          className="w-full py-3 text-sm font-bold text-green-600 hover:bg-green-50 transition-colors border-t border-slate-100 bg-white sticky bottom-0"
                        >
                ...
                <UserHoverCard userId={user.id}>
                  <Link to="/my-profile" className="h-8 w-8 rounded-full border-2 border-slate-200 overflow-hidden cursor-pointer hover:border-green-500 transition-colors block">
                ...
                <button 
                  ...
                  className="hidden sm:flex bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium text-sm items-center gap-2 transition-colors shadow-sm shadow-green-600/20"
                >
