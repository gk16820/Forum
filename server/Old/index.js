...
//             const result = await db.run('INSERT INTO users (username, email, password, avatar, role, domain, userType) VALUES (?, ?, ?, ?, ?, ?, ?)', [username, email, hashedPassword, avatar, role, domain, finalUserType]);
...
//             }
//             const result = await db.run('INSERT INTO posts (userId, title, question, tags, domain, image, communityId) VALUES (?, ?, ?, ?, ?, ?, ?)', [req.user.id, title, question, JSON.stringify(tags || []), domain || '', image || '', communityId || null]);
...
//         }
//     });
//     app.put('/api/posts/:id/vote', authenticateToken, async (req, res) => {
//         const { id } = req.params;
//         const { type } = req.body;
//         try {
...
//                 }
//                 if (domain)
//                     users = users.filter((u) => u.domain === domain);
...
//         }
//     });
//     const getPostWithMeta = async (postRow, user) => {
//         let userVote = null;
//         let isBookmarked = false;
//         if (user) {
...
//         }
//     });
//     app.get('/api/users/me/upvotes', authenticateToken, async (req, res) => {
//         try {
//             const rows = await db.all(`
//         SELECT p.*, u.id as authorId, u.username as authorName, u.description as authorDescription, u.avatar as authorAvatar 
//         FROM posts p 
...
//         }
//     });
//     app.get('/api/users/me/replies', authenticateToken, async (req, res) => {
//         try {
...
//         }
//     });
//     app.post('/api/bookmarks/:postId', authenticateToken, async (req, res) => {
//         try {
...
//         }
//     });
//     app.delete('/api/bookmarks/:postId', authenticateToken, async (req, res) => {
//         try {
...
//         try {
//             const result = await db.run('INSERT INTO comments (postId, userId, parentId, question) VALUES (?, ?, ?, ?)', [id, req.user.id, parentId || null, question]);
...
//                 await db.run('INSERT INTO notifications (userId, actorId, type, postId) VALUES (?, ?, ?, ?)', [post.userId, req.user.id, 'comment', id]);
//             }
...
//                 if (tagSeen.has(username.toLowerCase()))
//                     continue;
...
//                     await db.run('INSERT INTO notifications (userId, actorId, type, postId) VALUES (?, ?, ?, ?)', [taggedUser.id, req.user.id, 'mention', id]);
//                 }
//             }
//             res.status(201).json({ id: result.lastID });
//         }
...
//         }
//     });
//     app.get('/api/users/:id', optionalAuthenticateToken, async (req, res) => {
//         try {
...
//         try {
//             await db.run('UPDATE users SET avatar = ?, description = ?, location = ?, interests = ?, role = ?, domain = ? WHERE id = ?', [avatar || '/Blank profile.png', description, location, interests, role, domain, req.user.id]);
...
//         }
//     });
//     app.get('/api/communities', optionalAuthenticateToken, async (req, res) => {
//         try {
...
//         }
//     });
//     app.get('/api/communities/:id', optionalAuthenticateToken, async (req, res) => {
//         try {
//             const { id } = req.params;
