      ...
      <nav className="space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`mr-3 h-5 w-5 ${isCurrent ? 'text-blue-600' : 'text-slate-400'}`} />
              {item.name}
            </Link>
          );
        })}
      ...
      <div className="mt-10">
        ...
        <div className="space-y-2">
          {trending.length === 0 ? (
            ...
            <Link key={t.tag} to="/tags" className="flex items-center justify-between px-4 py-2 text-sm text-slate-600 hover:text-blue-600 hover:bg-slate-50 rounded-lg transition-colors">
              <span>{t.tag}</span>
